...
export default function HomePage() {
  ...
  return <div className="atelierPage atelierHome">
    ...
    {actionable.length>0 && <div className="atelierUrgent">{actionable.slice(0,3).map(job=>{const [label,action]=actionCopy(job);return <article key={job.id}><div className="atelierJobMeta"><span className="badge awaiting_owner_price">{label}</span><small>{age(job.updated_at || job.created_at)}</small></div><h3>{job.postcode || job.postcode_area || 'Location pending'}</h3><p>{job.tyre_size || 'Tyre details pending'}{job.tyre_quantity?` × ${job.tyre_quantity}`:''}</p><Link className="atelierButton primary" href={`/jobs/${job.id}`}>{action} ↗</Link></article>})}</div>}
    </section>
